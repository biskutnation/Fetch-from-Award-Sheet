...
//  • year 
//
//  look sheet_essentials() & categoryObject() at essentials.gs
...
function getHTMLcontent(data,essences) {
  ...
  const CATOBJ = categoryObject(essences.awdId); // it works

  // 210425 this dated method is hoping that row header won't move around or change its element
  // hope for a better object representation from a data row
  // because i really want to change/move certain columns place in spreadsheet
  // eg: remark columns shouldn't been there anymore
  ...
  if (!obj.PseudoName) obj.PseudoName = "-"
